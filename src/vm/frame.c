...
vm_frame_allocate (enum palloc_flags flags, void *upage)
{
  ...
  if (frame_page == NULL) {
    ...
    struct frame_table_entry *evic_entry = second_chance( thread_current()->pagedir );
    ...
    vm_frame_do_free(evic_entry->kpage, true); 
...
void vm_frame_do_free (void *kpage, bool free_flag){
