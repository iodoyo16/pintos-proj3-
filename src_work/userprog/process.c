...
{
  char *fn_copy = NULL;
  char *copy_of_cmd = NULL;
  //char copy_of_cmd[100];
  ...
  if (fn_copy == NULL)
    goto not_expected_outcome;
  strlcpy (fn_copy, file_name, PGSIZE);
  
  //make thread name
  copy_of_cmd = palloc_get_page(0);
  if(copy_of_cmd == NULL)
    goto not_expected_outcome;
  strlcpy(copy_of_cmd,file_name,PGSIZE);

  copy_of_cmd = strtok_r(copy_of_cmd," ",&save_ptr);
  for(int i=0;i<strlen(copy_of_cmd);i++){
    if(copy_of_cmd[i]==' ')
      copy_of_cmd[i]='\0';
  }
  if(filesys_open(copy_of_cmd)==NULL)
    return -1;
  ...
  tid = thread_create (copy_of_cmd, PRI_DEFAULT, start_process, pcb);
  if (tid == TID_ERROR)
    goto not_expected_outcome;
...
int parse_file_name(char *file_name, const char **parsed_filename_argv){
  char* token;
  char* save_ptr;
  int argc=0;
  token = strtok_r(file_name, " ", &save_ptr);
  while( token != NULL){
    parsed_filename_argv[argc++] = token;
